...
namespace CM
{
    ...
    public class ModBus : IDisposable
    {
        ...
        /// </summary>
        private static bool cmdRunning = false;
        /// <summary>
        /// Конструктор
        /// </summary>
        public ModBus(ComPortSettings _comPort)
        {
            ...
            #region Логирование 
            {
                string sEmul = (Program.cmdLineArgs.ContainsKey("NOBP")) ? "(EMUL)" : string.Empty;
            ...
            #endregion
            if (Program.cmdLineArgs.ContainsKey("NOBP"))
            {
        ...
        bool ReadInputRegister(byte _cmd, int _abonent, int _pos, out ushort _result)
        {
            ...
            else
            {
                try
                {
                    if(cmdRunning)
                    {
                        err = "Выполняется другая команда";
                        #region Логирование 
                        {
                    ...
                    if (Program.cmdLineArgs.ContainsKey("LOGMODBUS"))
                    {
                        #region Логирование 
                        {
                    ...
                    cmdRunning = true;
                    //ser.ReceivedBytesThreshold = 7;
                    lock (this)
                    {
                        ser.Write(query, 0, query.Length);
                        //Thread.Sleep(1000);
                        WaitHelper.Wait(1000);
                        //int ticks = System.Environment.TickCount + 2000;
                        //while (ser.BytesToRead<7)
                        //{
                        //    //Application.DoEvents();
                        //    Thread.Sleep(200);
                        //    if (System.Environment.TickCount > ticks) break;
                        //}
